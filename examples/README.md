...
本目录保存已经实际验证的可复现教程，包括三个联合仿真实验和一套 LubanCat 硬件接入流程。联合仿真文档的阅读起点是假定本机基础 ROS1 和 ROS2 环境已经构建好，但尚未创建具体实验；硬件文档则从一台新的 Ubuntu 电脑开始配置。

## 硬件接入教程

- [在另一台 Ubuntu 电脑上复用 LubanCat USB 桌面与共享网络](LubanCat/README.md)：为同一块 LubanCat-5 V2 配置新电脑上的 ADB、RNDIS 网络共享和 NoMachine 客户端。所有命令均明确区分新电脑与板子执行位置。
